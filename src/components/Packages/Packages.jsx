        ...
        <div className={styles.grid}>
          {packages.map((pkg, index) => (
            <div className={styles.package} key={index}>
              <h3>{pkg.title}</h3>
              <p className={styles.price}>Mulai dari {pkg.price}</p>
              <p>{pkg.channels}</p>
              ...
              <a href="https://wa.me/nomorwhatsapp" className={styles.ctaButton} target="_blank" rel="noopener noreferrer">Pesan Sekarang</a>
            </div>
          ))}
        </div>
      </div>
